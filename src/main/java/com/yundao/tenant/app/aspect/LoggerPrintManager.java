package com.yundao.tenant.app.aspect;
...
public class LoggerPrintManager {
	...
	@Pointcut("(execution(* com.yundao.tenant.app.controller..*.*(..))) && (!execution(* com.yundao.tenant.app.aspect..*.*(..)))")  
    ...
    public Object pringLog(ProceedingJoinPoint pjp) throws Throwable { 
    	 if(pjp == null ){
    		 logger.info("com.yundao.tenant.app.aspect.LoggerPrintManager [pringLog] param is null");
    		 return null;
    	 }
		 ...
		 if(retObject !=null && retObject instanceof Result){
			 try {
				...
				logger.error("com.yundao.tenant.app.aspect.LoggerPrintManager [pringLog] is error,cause is ",e);
			}
		 }
